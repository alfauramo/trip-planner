...
export function LoginPage() {
  ...
  useEffect(() => {
    if (justLoggedIn && user && profile) {
      ...
      const needsProfile = !profile.full_name && !profile.alias;
      navigate(needsProfile ? '/profile' : '/', { replace: true });
    }
      ...
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        ...
        {emailNotVerified && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-lg mb-4">
            <p className="font-medium mb-2">Email no verificado</p>
            <p className="text-sm mb-3">Hemos enviado un enlace de confirmación a tu email. Revisa tu bandeja de entrada y haz clic en el enlace para verificar tu cuenta.</p>
            {resent ? (
              <p className="text-sm text-green-600 font-medium">¡Email reenviado!</p>
            ) : (
              <button
                type="button"
