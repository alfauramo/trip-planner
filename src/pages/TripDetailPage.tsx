...
import { Receipt, Plus, Download, TrendingUp, ArrowRight, CheckSquare, Check, Trash2, Euro } from 'lucide-react';
...
export function TripDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'itinerary' | 'expenses' | 'members' | 'checklist' | 'map' | 'packing' | 'activity'
  >('itinerary');
  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
        ...
        <main className="flex-1 px-4 pt-5 pb-24 space-y-5">
          {activeTab === 'members' ? (
            ...
            <div className="h-[calc(100vh-260px)] bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800">
              <TripMap
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-8 w-full space-y-6">
        {activeTab === 'members' ? (
          ...
          <div className="h-[600px] bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800">
            <TripMap
              events={days.flatMap((d) => d.events)}
              onEventClick={(event) => {
                const day = days.find((d) => d.id === event.day_id);
    ...
    >
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            min="0"
            placeholder="Ej: 1500"
            className="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
        </div>
      </div>
      <div className="flex gap-3 pt-2">
        <button
          ...
          className="flex-1 bg-blue-500 text-white py-3 rounded-xl font-medium hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed"
        >
      ...
      <div className={cardCls}>
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
          />
          <button
            type="button"
            onClick={addItem}
            ...
            className="px-4 py-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 disabled:bg-gray-300 transition-colors"
          >
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3 px-4 py-3">
              <button
                ...
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${item.completed ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 dark:border-gray-600'}`}
              >
                {item.completed && <Check className="w-3 h-3" />}
              </button>
              <span
                className={`flex-1 text-sm ${item.completed ? 'text-gray-400 line-through' : 'text-gray-800 dark:text-white'}`}
              >
                {item.description}
              </span>
              <button
          ...
          {(Object.keys(EXPENSE_CATEGORIES) as ExpenseCategory[]).map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all flex items-center gap-1 ${category === cat ? `${catInfo.color} ring-2 ring-blue-500` : 'bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}
              >
          ...
          {members.map((m) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${isSelected ? 'bg-blue-500 text-white' : 'bg-white text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700'}`}
              >
                {getMemberDisplayName(m)}
              </button>
            );
...
function ExpensesSection({
  ...
  const categoryStats = eventsWithCost.reduce(
    (acc, e) => {
  ...
  const sortedCategories = catEntries
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {days.length === 0 ? (
            ...
            >
              <Plus className="w-4 h-4" /> Crear día
            </button>
          ) : (
            <>
              <button
                ...
                className="flex items-center gap-1.5 bg-blue-500 text-white px-4 py-2 rounded-xl font-medium text-sm hover:bg-blue-600"
              >
                <Plus className="w-4 h-4" /> {isMobile ? '' : 'Añadir gasto'}
              </button>
              {!isMobile && (
                <button
                  type="button"
                  onClick={() => exportToCSV(eventsWithCost, members, days)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium ${eventsWithCost.length > 0 ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-gray-50 text-gray-400 cursor-not-allowed opacity-50'}`}
        ...
        {tripBudget && tripBudget > 0 && (
          <div className="mt-3">
            ...
            <div className="h-2.5 bg-white/30 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${totalExpenses > tripBudget ? 'bg-red-400' : 'bg-white'}`}
                style={{ width: `${Math.min((totalExpenses / tripBudget) * 100, 100)}%` }}
              />
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        ...
        {memberStats.length === 0 ? (
          <div className={`${isMobile ? 'p-6' : 'p-8'} text-center text-sm text-gray-500`}>
          ...
          <div className="divide-y">
            {memberStats.map((stat) => (
              <div
                key={stat.userId}
                className={`${isMobile ? 'px-4 py-3' : 'px-5 py-4'} flex items-center justify-between`}
              >
                <div>
                  <p className="text-sm font-medium text-gray-800 dark:text-white">{stat.displayName}</p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    Pagó: {formatCurrency(stat.paid)} · Debe: {formatCurrency(stat.owed)}
                  </p>
                </div>
